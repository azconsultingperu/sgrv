...
- Corregir `app/templates/auth/login.html:14` que hace `{% raw %}{% set _ = get_flashed_messages(...) %}{% endraw %}` y vacía la cola antes de `base.html:44` (`#flashData`), para que los mensajes `danger` lleguen al JS de `main.js:347` y se genere `mc-toast`.
