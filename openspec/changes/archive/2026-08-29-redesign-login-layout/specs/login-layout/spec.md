...
La tarjeta `.login-card` SHALL tener `max-width` entre 380px y 420px (valor canónico 400px) y SHALL permanecer centrada horizontalmente con padding proporcional en `p-4 p-md-5` para inputs, sin cambiar `name/id` de campos.

#### Scenario: Ancho en desktop
- **WHEN** se carga `/auth/login` en viewport ≥ 992px
- **THEN** el ancho renderizado de `.login-card` es ≤ 420px y está centrado (margen lateral izquierdo ≈ derecho, diferencia < 8px) y los inputs mantienen padding lateral ≥ 12px.
