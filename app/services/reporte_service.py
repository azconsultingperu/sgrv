...
def generar_reporte_csv(tipo, params=None):
    ...
    if tipo == 'alumnos':
        ...
        query = Alumno.query.join(InstitucionEducativa, Alumno.institucion_id == InstitucionEducativa.id)
        for a in query:
        ...
        query = Visita.query.join(Alumno).join(Promotor)
        ...
        for ie in InstitucionEducativa.query.all():
            total = Alumno.query.filter_by(institucion_id=ie.id).count()
        ...
        for c in Carrera.query.all():
            total = Alumno.query.filter_by(carrera_id=c.id).count()
...
def generar_reporte_excel(tipo, params=None):
    ...
    if tipo == 'alumnos':
        ...
        for a in Alumno.query.join(InstitucionEducativa).all():
        ...
        for v in Visita.query.join(Alumno).join(Promotor).all():
